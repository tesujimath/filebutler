...
class Filter(object):
    ...
    def clearOwner(cls, f0):
        ...
        if f0 is None or f0.dataset is None and f0.sizeGeq is None and f0.mtime is None and f0.notPaths == [] and f0.regex is None:
            return None
        else:
            return cls(None, f0.dataset, f0.sizeGeq, f0.mtime, f0.notPaths, f0.regex)

    @classmethod
    def clearDataset(cls, f0):
        """Return a copy of f0 with no dataset specified, or None if f0 is None."""
        if f0 is None or f0.owner is None and f0.sizeGeq is None and f0.mtime is None and f0.notPaths == [] and f0.regex is None:
            return None
        else:
            return cls(f0.owner, None, f0.sizeGeq, f0.mtime, f0.notPaths, f0.regex)
    ...
    def clearSize(cls, f0):
        ...
        if f0 is None or f0.owner is None and f0.dataset is None and f0.mtime is None and f0.notPaths == [] and f0.regex is None:
            return None
        else:
            return cls(f0.owner, f0.dataset, None, f0.mtime, f0.notPaths, f0.regex)

    def __init__(self, owner=None, dataset=None, sizeGeq=None, mtime=None, notPaths=[], regex=None):
        self.owner = owner
    ...
    def intersect(self, f1):
        ...
        if self.owner is None:
            ...
            if self.owner == f1.owner:
                ...
                # incompatible, so set to something impossible, which we test for in consistent
        ...
        mtime = f1.mtime if self.mtime is None else self.mtime.intersect(f1.mtime)
        ...
        f2 = self.__class__(owner, dataset, sizeGeq, mtime, notPaths, regex)
        #debug_log("Filter(%s).intersect(%s)=%s\n" % (self, f1, f2))
        return f2

    @property
    def consistent(self):
        """Whether this filter is consistent.  An inconsistent filter matches nothing."""
        if self.owner is not None and '+' in self.owner:
            return False
        if self.dataset == '/':
            return False
        if self.mtime is not None and not self.mtime.consistent:
            return False
        return True
    ...
    def selects(self, filespec):
        ...
        if self.sizeGeq is not None and filespec.size < self.sizeGeq:
            return False
        if self.mtime is not None and not self.mtime.selects(filespec.mtime):
            return False
        if len(self.notPaths) > 0:
